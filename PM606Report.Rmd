...
The [HELIX (Human Early-Life Exposome, Vrijheid, M. et al., 2014)](https://www.projecthelix.eu/index.php/es/data-inventory) project investigates the comprehensive environmental exposures (exposome) from conception through childhood. It combines data on various environmental hazards affecting mothers and children, examining their impacts on health, growth, and development. Utilizing six European birth cohort studies (BiB, EDEN, INMA, KANC, MoBa, Rhea), HELIX integrates extensive existing data with new high-tech measurements of chemical, physical, and molecular exposures. Methods include smartphone-based pollution tracking and advanced lab techniques for detecting chemical contaminants. The study comprises three levels: a broad cohort of 31,472 mother-child pairs, a subcohort of 1,301 pairs with detailed biomarker and health outcome data at ages 6-11, and repeat-sampling studies involving 150 children and 150 pregnant women for personal exposure data collection. This multilevel approach helps in understanding the cumulative effect of various exposures on disease risk.

This project utilizes data from the subcohort of 1301 mother-child pairs in the HELIX study for whom complete exposure and outcome data were available. Exposure data included detailed dietary records after pregnancy and concentrations of various chemicals in child blood samples. There are categorical and numerical variables, which will include both demographic details and biochemical measurements. This dataset allows for robust statistical analysis to identify potential associations between chemical exposure and changes in BMI Z-scores, considering confounding factors such as age, gender, or socioeconomic status. There are no missing data so there is not need to impute the information. Child BMI Z-scores were calculated based on WHO growth standards.

In terms of adding the [metabolomic serum data](https://www.projecthelix.eu/images/HELIX_serum_metabol_DataSummary.pdf) with the rest of the dataset, 103 (about 8% of the data) values were excluded. Some data entries had no information when combined with the metabolomic data and therefore could not be imputed with the mean or median. While this may pose as a risk, including rows with missing values can lead to inaccurate or biased results. This is relevant since many statistical methods and machine learning algorithms require complete data to function correctly. The proportion of missing data of 8% is relatively small, making the exclusion of these rows a feasible approach. This minimizes the loss of valuable information while still maintaining a dataset of sufficient size for meaningful analysis.
...
```{r enet selection chemicals, warning=FALSE}
# ELASTIC NET
...
```{r diet selection enet, warning=FALSE}
...
Project HELIX. (n.d.). *Data Inventory*. Retrieved from <https://www.projecthelix.eu/index.php/es/data-inventory>

Siskos, A. P. et al. (2017). *HELIX-Serum Metabolomics*. Retrieved from <https://www.projecthelix.eu/images/HELIX_serum_metabol_DataSummary.pdf>

Vrijheid, M. et al. (2014). Early-life environmental exposures and childhood health: The HELIX project. *Environmental Health Perspectives, 122*(10), 995-1003. DOI: [10.1289/ehp.1307204](https://doi.org/10.1289/ehp.1307204)
